...
def get_args_parser():
    ...
    parser.add_argument('--config_file', '-c', type=str,
                        default=project_path + 'config/config_cfg_swinb.py',
                        )
...
def main(args):
    ...
    logger.debug("build model: {} ...".format(args.modelname))
    model, criterion, postprocessors = build_model_main(args)
    wo_class_error = False
    model.to(device)
    logger.debug("build model done: {}".format(args.modelname))
    ...
    logger.info('number of params before freezing: ' + str(n_parameters))
        ...
        for name, parameter in model.named_parameters():
            for keyword in args.freeze_keywords:
                if keyword in name:
                    ...
                    break
    n_parameters = sum(p.numel() for p in model.parameters() if p.requires_grad)
    logger.info('number of params after  freezing: ' + str(n_parameters))
